...
int main(int argc, char *argv[])
{
	GpuTimer timer;
	timer.Start();
	printf("ECSE 420 Lab 1: Logic Gates Simulation - parallel_unified_prefetch\n");
	if (argc != 4)
	{
	...
	if(read_csv_array(argv[1], cuda_file, &size)==0){
		...
		cudaMemPrefetchAsync(cuda_file, size*3*sizeof(int), deviceId);
		...
		cudaMemPrefetchAsync(cuda_output, size*sizeof(int), cudaCpuDeviceId);
